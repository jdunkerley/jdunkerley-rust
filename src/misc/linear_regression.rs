...
pub fn compute<I, T>(x_vals: I, y_vals: I) -> Option<(T, T)>
where
    ...
    T: Num + Copy,
{
    ...
    let slope = (accumulator.4 - accumulator.1 * accumulator.3 / accumulator.0)
        / (accumulator.2 - accumulator.1 * accumulator.1 / accumulator.0);
    let result = (
        slope,
        (accumulator.3 - slope * accumulator.1) / accumulator.0,
    );
